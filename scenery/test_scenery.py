...
#    if self.target_transform is not None:           #注释一下试试（2021/2/6）
...
def to_rgb(grayscale_input, ab_input, save_path):
...
def test_scenery():
  ...
  # test_imagefolder = GrayscaleImageFolder('D:/郭/大学课程/4-大四/毕业设计/照片上色/static/scenery/original/', test_transforms)
  test_imagefolder = GrayscaleImageFolder('./static/scenery/original/', test_transforms)
  test_loader = torch.utils.data.DataLoader(test_imagefolder, batch_size=1, shuffle=False)

  model = ColorizationNet()
  # model.load_state_dict(torch.load('D:/郭/大学课程/4-大四/毕业设计/照片上色/scenery/风景-model-epoch-11-losses-0.005062.pth'))
  model.load_state_dict(torch.load('./scenery/风景-model-epoch-11-losses-0.005062.pth'))
  model.eval()
