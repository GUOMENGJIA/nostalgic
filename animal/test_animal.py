...
def test_animal():
    ...
    # test_imagefolder = GrayscaleImageFolder('D:/郭/大学课程/4-大四/毕业设计/照片上色/static/animal/original/', test_transforms)
    test_imagefolder = GrayscaleImageFolder('./static/animal/original/',test_transforms)
    test_loader = torch.utils.data.DataLoader(test_imagefolder, batch_size=1, shuffle=False)

    model = ColorizationNet()
    # model.load_state_dict(torch.load('D:/郭/大学课程/4-大四/毕业设计/照片上色/animal/动物模型-model-epoch-75-losses-0.001594.pth'))
    model.load_state_dict(torch.load('./animal/动物模型-model-epoch-75-losses-0.001594.pth'))
    model.eval()
